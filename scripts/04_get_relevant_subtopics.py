...
def get_links(topic, all_topics):
    ...
    if (
        ...
        print(
            f"{topic} has already been saved to the wikipedia_pages folder.", end=" -> "
        )
    ...
    try:
        ...
        if (
            ...
            topic_check_pos_type(topic) == True and len(all_topics) == 1
        ):  # if the topic word is not a noun and there is only 1 topic
            print(
                f"{topic} is not a noun. Using it as a keyword to search for subtopics."
            )
            pass
        elif (
...
def main():
    ...
    print(
        f"N = {N} - The required minimum number of keywords in common between parent-child pages."
    )
    ...
    print(
        "Would you like to also generate a lorebook with the results of the subtopic finder? (y/n)"
    )
    choice = input("(y/n): ")
    if choice == "y":
