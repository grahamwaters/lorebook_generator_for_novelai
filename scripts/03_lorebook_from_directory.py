...
def create_keys(entries):
    ...
    for entry in tqdm(entries):
        ...
        for word, tag in preprocess(entry):
            if (tag == 'NNP' or tag == 'NNPS' or tag == 'CD') and word not in keys\
                and word not in stop_words and len(word) > 2 and word != "Gregorian": # remove stop words, and numbers greater than 2020 (which are probably years)
                try:
                    if int(word) < 2020:
                        continue
                except:
...
def main():
    ...
    print(f'   this will scan all {len(os.listdir("wikipedia_pages"))} docs for the topics you enter')
    while True:
        inp = input("Topic: ")
        if inp == "":
            break
